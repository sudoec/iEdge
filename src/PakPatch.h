...
)
{
    if (hFileMappingObject == resources_pak_map)
    {
        ...
        {
            LPVOID nbuffer = NULL;
            TraversalPakFile((BYTE*)buffer, (BYTE**)&nbuffer);
     ...
     //               const char prouct_title[] = u8R"({aboutBrowserVersion}</div><div class="secondary"><a target="_blank" href="https://github.com/sudoec/iEdge">Microsoft Edge Plus (iEdge).</a> )" RELEASE_VER_STR u8R"( inside</div>)";
     ...
     //       TraversalBrotliFile((BYTE**)&buffer, [=](uint8_t* begin, size_t size, size_t& new_len) {
     //           bool changed = false;

     //           BYTE search_start[] = R"(aboutObsoleteEndOfTheLine)";
     //           uint8_t* pos = memmem(begin, size, search_start, sizeof(search_start) - 1);
     //           if (pos)
     //           {
     //               // 压缩HTML以备写入补丁信息
     ...
     //               size_t aa = 0;
     //               BrotliCompress(html.length(), &aa, (uint8_t*)&html[0], begin + 8);
     //               plog(std::to_string(aa));
