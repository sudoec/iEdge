...
void plog(const std::wstring wstr)
{
    std::string strTmp(wstr.begin(), wstr.end());
    plog(strTmp);
}

void MyDebugView(std::string str)
{
    std::string Ret = "[W32DBG] " + str;
    OutputDebugStringA(Ret.c_str());
}

void MyDebugView(std::wstring str)
{
    std::wstring Ret = L"[W32DBG] " + str;
...
void SendKey(std::wstring &keys)
{
    ...
    for (auto& vk : vks)
    {
        SendMessage(GetForegroundWindow(), WM_KEYDOWN, vk, 0);
        SendMessage(GetForegroundWindow(), WM_KEYUP, vk, 0);
        //PostMessage(GetForegroundWindow(), WM_KEYDOWN, vk, 0);
        //PostMessage(GetForegroundWindow(), WM_KEYUP, vk, 0);
        //PostMessage(HWND_BROADCAST, WM_CHAR, vk, 0);
    }
    Sleep(20);
